...
#JUST TO CHECK-- Use code below to check the actual party names and abreviations
...
survey_clean_collect<-survey_clean |> 
  ...
  mutate(party_collect = 
           case_when(party_collect == "PSOE" ~ "PSOE",
                     ...
                     party_collect == "PP" ~ "PP",
                     party_collect == "P_N_VASCO" ~"P_N_VASCO",
                     party_collect == "CONV_UNIO" ~ "CONV_UNIO",
                     party_collect == "B_N_GALLEGO" ~ "B_N_GALLEGO",
                     party_collect == "VOX" ~ "VOX",
                     party_collect == "MP" ~ "MP",
                     party_collect == "MAS_PAIS" ~ "MAS_PAIS",
...
election_data_clean <- election_data_clean |> 
  left_join(abbrev_collect, by = c("party" = "denominacion")) #many parties in the elections dataset not found in abbrev

#just to test to see if I've gotten all of the party_collect variable correctly
#test <- election_data_clean |> 
  #filter(is.na(party_collect)) |> 
  #distinct(party) 
  

#removing variables, changing NA votes to 0
election_data_clean<-election_data_clean|> 
  mutate(date_elec = make_date(anno, mes)) |> 
  select(-tipo_eleccion, -codigo_distrito_electoral) |> 
  mutate(votes = ifelse(is.na(votes), 0, votes)) #CHANGING ALL NA VALUES IN VOTES TO 0!
...
election_data_clean_2 <-election_data_clean |> 
  ...
  mutate(votes_municipality = sum(votes)) |> #votes that each municipality casted for real
  ungroup() |> 
 ...
 q2_final_result <- election_data_clean_2 |> 
   ...
   select(date_elec, municipio, party, party_collect,  votes_perc_party_municipality) 
   
q2_final_result

```

### Alex - Q3 Which party was the second when the first was the PSOE? And when the first was the PP?
...
final_results <- bind_rows(
  mutate(parties_2nd_place_after_PSOE, source = "PSOE"),
  mutate(parties_2nd_place_after_PP, source = "PP") 
) |> 
  rename(election_winner = source)
...
election_data_clean_2 <-election_data_clean |> 
  ...
  ungroup() |> 
  mutate(votes_perc_party_municipality = (votes_party_munici/total_votes_municiplality)*100) |> 
  filter(censo >100000) |> 
  group_by(codigo_ccaa_pro_muni) |> 
...
ggplot(esp_can) +
  ...
  geom_sf(data = elec_winner_munic_2019a, aes(fill = party_collect), color = "black", size = 1) + 
...
election_winners <-   
  ...
  dplyr::select(municipio_id, vote_share, party_collect, party, anno, date_elec)  elec_winners_all_time <- munic |>
