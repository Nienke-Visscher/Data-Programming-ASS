...
survey_elec_sample<-survey_elec_sample |> 
  ...
  mutate(id_codigo_party = paste(id_codigo, date_elec, id_pollster, party_collect,sep = "-"))
  

duplicates_all_columns <- survey_elec_sample[duplicated(survey_elec_sample), ] #There are may duplictaes
...
The difference in voting intentions with the actual votes is computed by subtracting the voting intentions from the votes. This difference is then squared to release the negatives as we are interested in the total absolute count of error between the intended and the actual votes. Then, the square root of the mean of this difference grouped by election date has been computed. The results show that the polls of the elections of 11-2019 were most wrong.

```{r}

 survey_elec_sample<-survey_elec_sample |> 
  mutate(diff_vote_int = votes - vote_int) |> 
  mutate(diff_vote_int_2 =diff_vote_int^2) |> #As the difference can be both positive and negative, the differences in votes are squared to get rid of the negatives
  group_by(date_elec) |> 
  mutate(avg_diff_vote_int = sqrt(mean(diff_vote_int_2, na.rm = TRUE))) |> #Taking the squareroot to return the variable in to the orginal units
  ungroup() 
