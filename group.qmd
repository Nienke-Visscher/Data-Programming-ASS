 ...
 abbrev_collect<-abbrev |> 
  mutate(party_collect = case_when(
    str_detect(siglas, "PSOE") |
      ...
      str_detect(siglas, "psoe") ~ "PSOE", # PARTIDO SOCIALISTA OBRERO ESPAÑOL (beware: it has/had federations - branches - with some other name).
    
    str_detect(siglas, "PP") ~ "PP", # PARTIDO POPULAR
    ...
    str_detect(denominacion, "BILDU")|
      ...
      str_detect(denominacion, "VASCO") ~ "P_N_VASCO", # PARTIDO NACIONALISTA VASCO
      str_detect(denominacion, "UNIO") ~ "CONV_UNIO", # CONVERGÈNCIA I UNIÓ
      str_detect(siglas, "BNG") ~ "B_N_GALLEGO", # BLOQUE NACIONALISTA GALLEGO
      str_detect(siglas, "VOX") ~ "VOX", # VOX
      str_detect(siglas, "PAÍS") ~ "MP", # MÁS PAÍS
      str_detect(siglas, "COMPROMÍS") ~ "MAS_PAIS", # COMPROMÍS
      str_detect(siglas, "ERC") |
      str_detect(denominacion, "esquerra") ~ "ERC", # ESQUERRA REPUBLICANA DE CATALUNYA
    
    TRUE ~ "OTHER"))|> 
    distinct(denominacion, .keep_all = TRUE) 

 right_wing_parties <- c("PSOE", "PP", "VOX", "CS", "MP", "UP_IU")
...
#Only keeping 1 of the duplicate rows
...
CS = CUIDANOS, IU = UNIDAS_PODEMOS

Problem: There is no data on CIUDANOS, UNIDAS_PODEMOS and MP

```{r}
#Computing the average polling error per party per election
survey_elec_national_sample<-survey_elec_sample |> 
  filter(party_collect == "PSOE"|party_collect == "PP"|party_collect =="VOX"|party_collect =="CIUDADANOS "|party_collect =="MP"|party_collect =="UNIDAS_PODEMOS") |> 
  group_by(date_elec, party_collect) |> 
...
election_data_clean_lollipop <-
  ...
  mutate(votes_casted = votos_candidaturas + votos_blancos) |> 
  select(-votos_blancos, -votos_candidaturas) |> 
  mutate(municipio_id = paste(codigo_provincia, codigo_municipio, sep = "-")) |> 
  distinct(municipio_id, .keep_all = T) |> 
  select(municipio_id, codigo_ccaa, votes_casted, censo) |> 
...
# ordering regions by voter turnout
order_by_voter_turnout <- election_data_clean_lollipop %>%
  arrange(voter_turnout) |> 
  pull(codigo_ccaa)

election_data_clean_lollipop$codigo_ccaa <- factor(election_data_clean_lollipop$codigo_ccaa, levels = fct_relevel(order_by_voter_turnout))
```

Creating the lollipop graph

```{r}
ggplot(election_data_clean_lollipop, aes(x = voter_turnout, y = codigo_ccaa)) + 
  geom_segment(aes(xend = 0, yend = codigo_ccaa), color = "black") +
  geom_point(position = position_dodge2(width = 0.5), color = "black", size = 2) +
  geom_point(data = filter(election_data_clean_lollipop, codigo_ccaa == "00"),
             aes(x = voter_turnout, y = codigo_ccaa),
             color = "red", size = 3) +
   geom_segment(data = filter(election_data_clean_lollipop, codigo_ccaa == "00"),
               aes(x = voter_turnout, y = codigo_ccaa, xend = 0, yend = codigo_ccaa),
               color = "red") + 
  scale_x_continuous(expand =expansion(mult = c(0, .08)),
                     breaks = seq(0, 100, by = 10)) +
  labs(title = "Voter Turnout by Region (April 2019)",
       x = "",
       y = "") +
  theme_minimal() +
  geom_text(aes(label = sprintf("%.2f%%", voter_turnout)),
            position = position_dodge(width = 0.5),
            hjust = -0.2, vjust = 0.5, size = 3) +
  scale_y_discrete(labels=c("00" = "Spain",
                            "01"="Andalucia",
                            "02"="Aragon",
                            "03"="Asturias",
                            "04"="Baleares",
...
# my attempt at changing y-axis label color (in theme command)

axis.text.y = element_text(color = c(ifelse(election_data_clean_lollipop$codigo_ccaa == "00", "red", "black")))
```
