...
#Creating a new column with the collected parties
 abbrev_collect<-abbrev_test |> 
  mutate(party_collect = case_when(
    ...
    str_detect(denominacion, "CIUDADANOS-") |
    str_detect(denominacion, "PARTIDO DE LA CIUDADANÍA")  ~ "CS", #CIUDADANOS (caution: has/had federations - branches - with some other name)
    ...
    str_detect(denominacion, "BILDU")|
      ...
      str_detect(siglas, "BNG") ~ "BNG", # BLOQUE NACIONALISTA GALLEGO
    
      str_detect(siglas, "VOX") ~ "VOX", # VOX
    
      str_detect(siglas, "MÁS PAÍS") ~ "MP", # MÁS PAÍS
