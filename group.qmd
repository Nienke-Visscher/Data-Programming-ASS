...
plot1 <- ggplot(data = graph_data, aes(x = date_elec, y = votes_elec_party_perc, color = party_type, group = party_type)) +
  geom_line() +
...
In this section I took the national parties and compared them to all the other parties of interest. The plot at the end shows the relationship of the share going to each group. We see that in 2016 the share of votes going to national parties increased, while the opposite happened to regional and nationalist parties.
...
election_data_clean_2 <-election_data_clean |> 
  ...
  mutate(votes_nul_blanc_total = votos_blancos+ votos_nulos) |> #votes that each municipality casted that didnt count
  mutate(votes_municipality = sum(votes)) |> #votes casted in each municipality 
  ungroup() |> 
  mutate(total_votes_municipality = votes_nul_blanc_total + votes_municipality) |> #total votes casted in each municipality for every row
  group_by(codigo_ccaa_pro_muni, party_collect) |> 
  mutate(votes_party_municipality = votes) |> #for each party in each municipality total votes
  ungroup() |> 
  mutate(votes_perc_party_municipality = (votes_party_municipality/total_votes_municipality)*100) |> #percentage of votes that a party got from the total amount of votes cast in each municipality
  ungroup()
 ...
 q2_final_result <- election_data_clean_2 |> 
   ...
   select(date_elec, municipio, party_collect, censo,  votes_perc_party_municipality) 
...
Barplot for if PSOE wins
```{r}
ggplot(election_data_mika, 
       aes(x = convocatorias, 
           y = vote_share, 
           fill = party_collect)) +
...
election_data_turnout <- election_data_clean |> 
  ...
  mutate(
    turnout_pcg = round(((votos_candidaturas/censo)*100), digits = 2)) |> 
  ungroup() |> 
  mutate(
    party_collect=party_collect.x) |> 
  group_by(anno, party_collect) |> 
  mutate(
    sum_votes = sum(votes))

#Low turnout dataset
low_turnout_data <- election_data_turnout |> 
  mutate(party_collect = party_collect.x) |> 
  mutate(sum_votes = sum_votes) |>  # make sure sum_votes is present
  select(anno, censo, votes, party, party_collect, turnout_pcg, sum_votes) |> 
  filter(turnout_pcg < 50, votes > 0)


plot_low <- ggplot(low_turnout_data, aes(x = party_collect, y = sum_votes, fill = party_collect)) +
  geom_bar(stat = "identity") +
  labs(title = "Low turnout",
       x = "Party",
       y = "")+
 theme(axis.text.y = element_blank())

#High turnout database
high_turnout_data <- election_data_turnout |> 
  mutate(party_collect = party_collect.x) |> 
  mutate(sum_votes = sum_votes) |>  # make sure sum_votes is present
  select(anno, censo, votes, party, party_collect, turnout_pcg, sum_votes) |> 
  filter(turnout_pcg >= 50, votes > 0)
...
plot_high <- ggplot(high_turnout_data, aes(x = party_collect, y = sum_votes, fill = party_collect)) +
  geom_bar(stat = "identity") +
  labs(title = "High turnout",
       x = "Party",
       y = "")+
 theme(axis.text.y = element_blank())
...
plot_high + plot_low + plot_layout(nrow = 2)
```

### Q5 Who benefits from low turnout?
```{r}
#Subset with rural municipalities
election_data_rural<- election_data_clean |> 
  mutate(party_collect = party_collect.x) |> 
  select(anno, censo, votes, party, party_collect) |> 
  filter(censo < 30000, votes > 0) |> 
  group_by(party_collect, anno) |> 
  mutate(
    sum_votes = sum(votes)
  )

#Subset with urban municipalities
election_data_urban <- election_data_clean |> 
  mutate(party_collect = party_collect.x) |> 
  select(anno, censo, votes, party, party_collect) |> 
  filter(censo > 30000, votes > 0) |>
  group_by(party_collect, anno) |> 
...
#Plots
plot_rural <- ggplot(election_data_rural, aes(x = party_collect, y = sum_votes, fill = party_collect)) +
  geom_bar(stat = "identity") +
  labs(title = "Votos por Partido en Áreas Rurales",
       x = "Partido",
       y = "")+
 theme(axis.text.y = element_blank())+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

plot_urban <- ggplot(election_data_urban, aes(x = party_collect, y = sum_votes, fill = party_collect)) +
  geom_bar(stat = "identity") +
  labs(title = "Votos por Partido en Áreas Urbanas",
       x = "Partido",
       y = "")+
 theme(axis.text.y = element_blank())+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
plot_rural + plot_urban + plot_layout(nrow = 2)
```

### Q6
```{r}
election_data_areas <- election_data_turnout |> 
  mutate(area_type = case_when(
    censo < 30000 ~ "Rural",
    censo >= 30000 ~ "Urban")) |> 
  mutate(
    area_category = case_when(
      turnout_pcg >= 50 & area_type == "Urban" ~ "High Turnout Urban",
      turnout_pcg >= 50 & area_type == "Rural" ~ "High Turnout Rural",
      turnout_pcg < 50 & area_type == "Urban" ~ "Low Turnout Urban",
      turnout_pcg < 50 & area_type == "Rural" ~ "Low Turnout Rural"
    )
  )


election_data_areas <- election_data_areas %>%
  mutate(vote_diff = case_when(
    area_type == "High Turnout Urban" ~ ifelse(party == "OTHER", 0.2, ifelse(party %in% c("PP", "PSOE") | party == "UP_IU", -0.1, 0)),
    area_type == "High Turnout Rural" ~ ifelse(party == "PSOE", 0.1, ifelse(party %in% c("PP", "PSOE") | party == "UP_IU", -0.1, 0)),
    area_type == "Low Turnout Urban" ~ ifelse(party == "OTHER", -0.1,ifelse(party %in% c("PP", "PSOE") | party == "UP_IU", 0.1, 0)), 
    area_type == "Low Turnout Rural" ~ ifelse(party == "PSOE", 0.05, ifelse(party %in% c("PP", "PSOE") | party == "UP_IU", 0, 0))
  ))
```
...
### Election winners map

#### Election Winners November 2019
...
#### Looking at multiple elections at a time
