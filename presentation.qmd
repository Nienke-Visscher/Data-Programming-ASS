...
p1<-ggplot(data = graph_data, aes(x = date_elec, y = votes_elec_party_perc, color = party_type, group = party_type)) +
  geom_line() +
...
election_data_turnout <- election_data_clean |>  
  ...
  mutate(vote_share = (votes / (votos_candidaturas + votos_blancos+votos_nulos))*100) |>
  ungroup() 
...
#Computing the average polling error per polling house per election and obtaining the minimum and maximum polling with their corresponding pollhouses per election
survey_elec<-survey_elec |> 
...
## Dumbell graph data preperation I
...
## Dumbell graph data preperation II
...
## Dumbell graph - voter turnout differences
...
election_data_clean_red_winner_2019_b <-
  election_data_clean |>  
  dplyr::select(-anno, -mes, -vuelta,-participacion_1, -participacion_2) |>  
...
election_winners <-   
  election_data_clean |>
  dplyr::select(-mes, -vuelta,-participacion_1, -participacion_2) |> 
  drop_na(votes) |>
  mutate(municipio_id = paste(codigo_provincia, codigo_municipio, sep = "-")) |> 
  group_by(date_elec, municipio_id) |> 
  mutate(vote_share = (votes / (votos_candidaturas + votos_blancos + votos_nulos))*100) |>
